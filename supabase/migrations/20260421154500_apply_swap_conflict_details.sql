-- Add richer conflict diagnostics to apply_swap_request so UI can show
-- exactly which existing shift blocks the swap.
...
declare
  ...
  v_conflict_shift public.shifts%rowtype;
begin
  v_actor := auth.uid();
  ...
  if v_requester_shift.user_id = v_request.requester_user_id
    ...
    if found then
      raise exception
        'Swap cannot be applied: target user already has a shift in requester slot (conflict_shift_id=%, user_id=%, starts_at=%, ends_at=%)',
        v_conflict_shift.id,
        v_conflict_shift.user_id,
        v_conflict_shift.starts_at,
        v_conflict_shift.ends_at;
    end if;
    ...
    if found then
      raise exception
        'Swap cannot be applied: requester already has a shift in target slot (conflict_shift_id=%, user_id=%, starts_at=%, ends_at=%)',
        v_conflict_shift.id,
        v_conflict_shift.user_id,
        v_conflict_shift.starts_at,
        v_conflict_shift.ends_at;
