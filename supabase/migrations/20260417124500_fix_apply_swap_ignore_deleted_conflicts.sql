-- Fix false-positive conflict checks in apply_swap_request.
--
-- We keep historical rows in public.shifts with status='deleted' for
-- reconciliation/audit. Those rows must not block a new swap when checking
-- slot collisions. Only active rows should participate in conflict checks.
