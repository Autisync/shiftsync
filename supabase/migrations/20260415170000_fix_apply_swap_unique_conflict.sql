-- Fix apply_swap_request to avoid transient unique collisions on
-- shifts(user_id, starts_at, ends_at) when swapping two rows.
...
begin
  ...
  if v_requester_shift.user_id = v_request.requester_user_id
    ...
    -- Single statement swap avoids transient unique collisions.
  ...
  -- Idempotent case: shifts are already swapped.
