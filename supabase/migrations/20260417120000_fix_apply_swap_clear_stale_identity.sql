-- After a swap is applied the two shift rows move to new owners.
-- Their google_event_id and shift_uid both embed the old owner's identity,
-- so the next calendar sync looks for those events in the new owner's
-- Google Calendar, doesn't find them, and deletes them.
--
-- Fix: when ownership changes, clear google_event_id and regenerate
-- shift_uid with the new owner so syncs treat them as fresh events.
...
begin
  ...
  if v_requester_shift.user_id = v_request.requester_user_id
    ...
    -- Detect real conflicts with third-party rows before swapping.
    if exists (
      select 1
    ...
    set user_id = case
        ...
        -- Recompute shift_uid for the new owner so tracked-record lookups
        -- do not resolve to the previous owner's sync keys.
        shift_uid = case
        ...
        -- Clear the stale Google event reference so the next sync creates
        -- a fresh event in the new owner's calendar instead of deleting it.
