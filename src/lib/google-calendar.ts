 ...
 */
export function buildCalendarEventPayload(shift: ShiftData): Partial<CalendarEvent> {
  ...
  if (shift.employeeName) descriptionParts.push(`Employee: ${shift.employeeName}`);
...
export class GoogleCalendarService {
  ...
  async listCalendars(): Promise<GoogleCalendar[]> {
    const data = await this.fetchAPI<{ items?: GoogleCalendar[] }>('/users/me/calendarList');
    return data.items || [];
  }
  ...
  async createCalendar(
    ...
    const data = await this.fetchAPI<{ id: string; summary: string }>('/calendars', {
      method: 'POST',
  ...
  async createEvent(calendarId: string, shift: ShiftData): Promise<CalendarEvent> {
    return this.fetchAPI<CalendarEvent>(`/calendars/${encodeURIComponent(calendarId)}/events`, {
      method: 'POST',
  ...
  async deleteEvent(calendarId: string, eventId: string): Promise<void> {
    try {
      ...
      if (status === 404 || status === 410 || message.includes('resource has been deleted')) {
        return;
      }
      throw error;
    }
