 ...
 * SwapHRActionPage
 *
 * Landing page for HR managers clicking approve/decline links from swap
 * notification emails. URL format: /home/swaps/action?token=<token>&action=approve|decline
 */

import { useEffect, useState } from "react";
import { useSearchParams } from "react-router-dom";
import type { SwapService } from "@/services/backend/types";
import { Button } from "@/components/ui/button";

interface SwapHRActionPageProps {
  service: SwapService;
}

type PageState = "loading" | "success" | "error" | "invalid";

export function SwapHRActionPage({ service }: SwapHRActionPageProps) {
  ...
  useEffect(() => {
    ...
    const perform = async () => {
      try {
        await service.processHrDecisionAction({
          token,
  ...
  const actionLabel = action === "approve" ? "aprovado" : "recusado";
  const actionColor =
          ...
          <>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                A sua decisão foi registada com sucesso. Os colaboradores serão
                notificados no ShiftSync.
              </p>
            </div>
